...
developers:['KATHYA AI Developers','Build conversational agents, actions, events and business integrations with KATHYA.'],
...
export default function Seo({route}){useEffect(()=>{const m=meta[route]||meta.home;document.title=m[0];let d=document.querySelector('meta[name="description"]');if(!d){d=document.createElement('meta');d.name='description';document.head.appendChild(d)}d.content=m[1];let c=document.querySelector('link[rel="canonical"]');if(!c){c=document.createElement('link');c.rel='canonical';document.head.appendChild(c)}c.href=base+(route==='home'?'/':'/'+route+'/');},[route]);return null}